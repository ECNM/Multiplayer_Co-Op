...
public class Monster : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        night = timer[0].GetComponent<Timer>().night;
        ...
        if (hp <= 0)
        {
            ...
            moveSpeed = 0;
            if (!transform.name.Equals("NightBorne(Clone)"))
            {
        ...
        else if (night && !transform.name.Equals("NightBorne(Clone)"))
        {
            action = false;
            if (Vector2.Distance(player[index].transform.position, transform.position) > distance)
            {
        ...
        if (action)
        {
            GetComponent<ActionM>().enabled = true;
        }
        else
        {
            GetComponent<ActionM>().enabled = false;
        }
    }
    ...
    /*IEnumerator WaitForFunction()
    {
        while (action)
        {
            ...
            if (xcount == 2)
            {
                int xcount2 = UnityEngine.Random.Range(1, 3);
                animatorController.SetBool("Move", true);
                if (xcount2 == 1)
                {
                    Debug.Log(transform.name + "Walk");
                    transform.Translate(new Vector2(-1 * moveSpeed - ((moveSpeed / 100) * 60), 0) * Time.deltaTime);
                    GetComponent<SpriteRenderer>().flipX = false;
                }
                if (xcount == 2)
                {
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.name.Equals("WallM"))
        {
            hp = setHp;
        }
    }
    ...
    public void Heart()
    {
        ...
        players[0].GetComponent<PlayerControl>().numOfhearts += 1;
        players[1].GetComponent<PlayerControl>().numOfhearts += 1;
